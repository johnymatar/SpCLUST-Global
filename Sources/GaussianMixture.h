...
double GaussianMixture::icl(){
	/*
	Integrated classification/complete likelihood for the current model on the input X.
	...
	*/
	//cout<<" ICL() called ";
	double** probs, ENT=0.0;
	probs=new double*[this->n_samples];
	for(int i=0; i<this->n_samples;i++)
		probs[i]=new double[this->n_components];

	predict_proba(this->X, probs, this->n_samples, this->n_features);

	for(int i=0; i<this->n_components; i++){
		for(int j=0; j<this->n_samples; j++){
			ENT-=(probs[j][i]*log(probs[j][i]));
		}
	}
	//cout<<ENT;
	for(int i=0; i<this->n_samples;i++)
		delete[] probs[i];
	delete[] probs;
	return (-2 * this->log_likelihood() * this->n_samples + this->n_parameters() * log(this->n_samples)) + 2 * ENT;
} 
